...
public:
    ...
    void writeRelocatableToFile(const string &fileName);

    void writeExecutableToFile(const string &filename);
    ...
    string sectionName(const Elf32_Sym &sym) const {
        return sectionName(sectionTable.sectionDefs[sym.st_shndx]);
    }

    string sectionName(Elf32_Section sec) const {
        return sectionName(sectionTable.sectionDefs[sec]);
    }

    Elf32_Sym &sectionSymbol(Elf32_Section sec) {
        return symbolTable.symbolDefinitions[sectionTable.sectionDefs[sec].sh_name];
    }

    friend ostream &operator<<(ostream &os, ElfFile &file);
...
private:
    void loadRelocatableFile(fstream &file);
